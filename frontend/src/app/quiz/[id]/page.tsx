...
const TRIVIA_API_URL = process.env.NEXT_PUBLIC_TRIVIA_API_URL || 'https://the-trivia-api.com/api';
...
export default function QuizPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: questions = [], isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`${TRIVIA_API_URL}/questions?categories=${id}&limit=15`);
  ...
  const handleNext = async () => {
    ...
    const newAnswers = { ...userAnswers, [currentQuestion.id]: selectedOption || 'Unanswered' };
  ...
  if (!questions.length) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-background">
        <p className="text-foreground text-sm mb-4">Failed to initialize session. Upstream API may be unavailable.</p>
        <Button onClick={handleReset} variant="outline">Exit</Button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen py-10 px-6 max-w-2xl mx-auto flex flex-col bg-background selection:bg-primary selection:text-primary-foreground cursor-default">
      ...
      <div className="flex justify-between items-center pt-6 border-t border-border mt-auto">
        <Button variant="ghost" onClick={handleReset} className="gap-2 -ml-2 text-muted-foreground hover:text-foreground">
